...
def write_data_to_json(nameFile: str, url, typeOfDoing: str):
    with open(nameFile, typeOfDoing) as file:
        json.dump(url, file, indent=4, ensure_ascii=True)


"Сохраняем данные в JSON файл"
# write_data_to_json('full_information.json', full_information_dict, 'w')
...
def getAllData():
    ...
    for sector_names, sector_hrefs in allInfo.items():
        ...
        rep = [',', ' ', '-', '/']
        for item in rep:
            if item in sector_names:
                sector_names = sector_names.replace(item, '_')
        ...
        write_data_to_json(f"data_json/{counter}_{sector_names}.json", infoAboutCoin_json, 'w')
        ...
        if iteration == -1:
            cowsay.cow("Сбор данных завершен!!!")
